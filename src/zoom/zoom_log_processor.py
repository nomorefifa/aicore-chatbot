...
def _gemini_group_names(names: list[str]) -> dict[str, str]:
    ...
    try:
        ...
        # 특수 형식 감지 → 프롬프트에 동적으로 주의사항 추가
        extra_notes = []
        extra_examples = []

        # 숫자 식별자 패턴 감지 (접미사/접두사/구분자 포함 다양한 형태)
        # 예: 김찬우8621 / 김소영_6737 / 김소영 6737 / 20251340김소영 / 22200097_김성민
        _has_number_id = any(
            re.search(r"[가-힣][\s_]?\d{3,}", n)   # 이름 뒤 숫자 (붙음/공백/언더스코어)
            or re.search(r"\d{5,}[\s_]?[가-힣]", n) # 학번 등 긴 숫자 앞에 이름
            for n in names
        )
        if _has_number_id:
            extra_notes.append(
                "일부 이름에는 전화번호 뒷자리 또는 학번 등 개인 식별 번호가 포함되어 있습니다 "
                "(예: 김찬우8621 / 김소영_6737 / 20251340김소영). "
                "이 숫자는 사람마다 고유한 식별자이므로, 한글 이름이 동일하더라도 "
                "숫자가 조금이라도 다르면 반드시 다른 사람입니다. 절대 병합하지 마세요."
            )
            extra_examples.append('- "김민정1184"와 "김민정9977" → 숫자 다름 → 병합 X (동명이인)')
            extra_examples.append('- "이수민4304"와 "이수민9076" → 숫자 다름 → 병합 X (동명이인)')
            extra_examples.append('- "김찬우8621"와 "김찬우8621" → 숫자까지 완전히 동일 → 병합 O')
            extra_examples.append('- "김찬우8621"와 "김찬우8622" → 이름 같고, 숫자도 862까진 같지만 마지막 1/2가 다름 이는 오타로 판단 → 병합 O')
            extra_examples.append('- "김찬우8621"와 "김찬유8621" → 숫자가 동일하지만 이름 마지막 글자가 우/유로 나뉨 이는 오타로 판단 → 병합 O')

        extra_notes_str = ("\n\n[추가 주의사항]\n" + "\n".join(extra_notes)) if extra_notes else ""
        extra_examples_str = ("\n" + "\n".join(extra_examples)) if extra_examples else ""
...
5. 확실하지 않으면 병합하지 말고 별도 그룹으로 유지하세요.{extra_notes_str}
...
- "천안청수초 배은주"와 "천안청수초 류병선" → 사람이름 다름 → 병합 X{extra_examples_str}
