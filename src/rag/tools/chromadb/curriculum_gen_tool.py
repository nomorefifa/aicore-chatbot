...
def _build_curriculum_system_instruction() -> str:
    ...
    return (
        ...
        "[콘텐츠 밀도 기준 - 반드시 준수]\n"
        "각 단계의 세부 항목 수는 해당 단계의 시간에 비례해야 합니다.\n"
        "- 1H 단계: 최소 3개 세부 항목\n"
        "- 2H 단계: 최소 5개 세부 항목\n"
        "- 3H 이상 단계: 시간당 최소 3개, 즉 3H→9개 이상, 4H→12개 이상\n"
        "세부 항목이 기준에 미치지 못하면 출력이 불완전한 것으로 간주합니다.\n\n"

        "[실습 항목 작성 기준 - 반드시 준수]\n"
        "실습이 포함된 단계에서 실습 항목은 반드시 아래 형식으로 작성하세요.\n"
        "  [실습] {실습 제목}\n"
        "    - 사용 툴/환경: {구체적인 도구명, 버전, 플랫폼}\n"
        "    - 실습 내용:\n"
        "        1. {단계별 구체적 실습 내용}\n"
        "        2. {단계별 구체적 실습 내용}\n"
        "        3. {단계별 구체적 실습 내용}\n"
        "    - 산출물: {실습 완료 후 결과물}\n"
        "실습 항목을 단순히 '실습: OO 실습'으로만 표기하는 것은 금지합니다.\n\n"
        ...
        "■ 규칙\n"
        "- 세부 항목 수는 반드시 [콘텐츠 밀도 기준]을 충족해야 합니다\n"
        "- 실습 항목은 반드시 [실습 항목 작성 기준] 형식을 따라야 합니다\n"
        "- 각 단계 시간의 합이 총 교육 시간과 정확히 일치해야 합니다\n"
        "- 도구명, 라이브러리명, 플랫폼명은 반드시 구체적으로 명시하세요\n"
